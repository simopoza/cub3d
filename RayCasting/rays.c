...
/*   Updated: 2023/01/12 03:04:07 by flouta           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void cast_all_rays(t_infos *data)
{
	...
	float ray_angle;
 	i = 0;
	ray_angle = data->player.angle - ( data->player.view / 2);
...
void cast_each_ray(t_infos *data , float ray_angle, int ray_index)
{
	...
	//vertical
		...
		float horzHitDistance = h_find
        ? distanceBetweenPoints(data->player.x_pos, data->player.y_pos, h_next_x, h_next_y) : MAXFLOAT;
    float vertHitDistance = v_find
        ? distanceBetweenPoints(data->player.x_pos, data->player.y_pos, v_next_x, v_next_y) : MAXFLOAT;

    if (vertHitDistance < horzHitDistance) {
        data->rays[ray_index].distance = vertHitDistance;
        data->rays[ray_index].wall_x = v_next_x;
        data->rays[ray_index].wall_y = v_next_y;
    } else {
        data->rays[ray_index].distance = horzHitDistance;
